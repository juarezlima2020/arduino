...
const RGB DayDefaultColor = {   0,  0, 80 };
const RGB TodayColor      = { 255, 50,  0 };
...
#define PIN            2 // Clock LED strip pin (use GPIO 2)
#define PIN_WEEKDAYS   0 // Weekdays LED strip pin
...
//************* OTA Setup Function ******************************
void setupOTA() {
  ArduinoOTA.setHostname("WolClock_004_ESP32_JERSON");
...
void SetBrightness(time_t t) {
  ...
  if ((weekday() >= 2 && weekday() <= 6 && (NowHour > WeekNight.Hour || (NowHour == WeekNight.Hour && NowMinute >= WeekNight.Minute) || (NowHour == WeekMorning.Hour && NowMinute <= WeekMorning.Minute) || NowHour < WeekMorning.Hour)) ||
      ((NowHour > WeekendNight.Hour || (NowHour == WeekendNight.Hour && NowMinute >= WeekendNight.Minute) || (NowHour == WeekendMorning.Hour && NowMinute <= WeekendMorning.Minute) || NowHour < WeekendMorning.Hour) && (weekday() == 1 || weekday() == 7))) {
